# -*- coding: utf-8 -*-
"""
Created on Sun Oct 16 12:09:54 2016
...
class cannon_trajectory:
    ...
    def calculate(self):
        ...
        while self.y[-1] >= 0:
            ...
            vx_N = self.vx[-1] - math.e**(-self.y[-1]/10000)*self.v[-1]*(4*(10)**(-5))*self.vx[-1]*self.dt
            y_N = self.y[-1] + self.vy[-1] * self.dt
            vy_N = self.vy[-1] - self.g*self.dt - math.e**(-self.y[-1]/10000)*self.v[-1]*(4*(10)**(-5))*self.vy[-1]*self.dt
...
py.title('Isothermal Ideal Gas Drag')
py.show()